...
def loadDefaultNLP(is_big: bool = True) -> Any:
    ...
    spacy.language.Language.component("segment_on_newline", func=segment_on_newline)
...
# Define the segment_on_newline component
def segment_on_newline(doc, nlp, name):
    for token in doc[:-1]:
        if token.text.endswith("\n"):
            doc[token.i + 1].is_sent_start = True
    return doc

# Register the segment_on_newline component with SpaCy
spacy.language.Language.component("segment_on_newline", func=segment_on_newline)
