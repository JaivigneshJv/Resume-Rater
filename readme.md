...
Test run
    ...
    python3 main.py --type fixed "src/data/test/New-York-Resume-Template-Creative.pdf" --model_name model


## Usage
### For testing:
Test resumes can be found in `src/data/test`
To use the app, send a POST request to the /rate endpoint with a JSON payload containing the path to the resume file, the type of model to use, and the name of the model file. For example:
```
{
...
https://github.com/ongteckwu/Resume-Rater